...
This skill is instructions only. It assumes the `browser-harness` command is on `$PATH`. If `command -v browser-harness` fails, do the one-time install in [references/install.md](references/install.md) first, then continue. Installing clones the browser-harness repo to a durable location (the "checkout" referenced below). Once `browser-harness <<'PY' … PY` prints page info, never run install/connection steps again as part of normal work.
...
## Deeper mechanics live in your browser-harness checkout

The install prerequisite clones the full `browser-harness` repo. When you struggle with a specific UI mechanic, read the matching file in that checkout's `interaction-skills/` directory before inventing an approach: browser-wall, connection, cookies, cross-origin-iframes, dialogs, downloads, drag-and-drop, dropdowns, iframes, network-requests, print-as-pdf, profile-sync, screenshots, scrolling, shadow-dom, tabs, uploads, viewport.

For task-specific helper additions, edit `agent-workspace/agent_helpers.py` in that checkout. Community per-site playbooks live in `agent-workspace/domain-skills/<host>/` and are **off by default** — set `BH_DOMAIN_SKILLS=1` to enable them.
...
- Prefer compositor-level actions (screenshots, coordinate clicks, raw key input) over framework/DOM hacks.
