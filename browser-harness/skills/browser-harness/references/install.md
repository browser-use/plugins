...
Editable (`-e`) keeps the command global while pointing at the real checkout, so edits to `agent-workspace/agent_helpers.py` take effect on the next call. Prefer a durable path (e.g. `~/Developer/browser-harness`), not `/tmp`. This checkout is also where `interaction-skills/` and `agent-workspace/` live — the skill points you back to it.
...
The full connection reference and troubleshooting live in the cloned repo's `install.md`.
